...
pdf:
	...
	# rticles overwrites RJwrapper.tex on every render. Re-apply the
	# "Research article" patch and recompile directly with xelatex so
	# the final PDF reflects the patched wrapper and suppressed footer.
	sed -i '' 's/Contributed research article/Research article/' paper/rj/RJwrapper.tex
	cd paper/rj && xelatex -interaction=nonstopmode RJwrapper.tex >/dev/null 2>&1 || true
	cd paper/rj && bibtex RJwrapper >/dev/null 2>&1 || true
	cd paper/rj && xelatex -interaction=nonstopmode RJwrapper.tex >/dev/null 2>&1 || true
	cd paper/rj && xelatex -interaction=nonstopmode RJwrapper.tex >/dev/null 2>&1 || true
